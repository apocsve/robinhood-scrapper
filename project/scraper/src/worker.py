...
def cli(mode: str, rabbitmq_host: str, rabbitmq_port: str, worker_request_cooldown_seconds: float):
    if mode in ["quote", "fundamentals"]:
        ...
        logged = TRADER.login(username=robinhood_username, password=robinhood_password, qr_code=mfa_secret)
        print(" [x] Logged %r" % logged)
    ...
    def handle_work(_channel, _method, _properties, body):
        ...
        rabbitmq_channel.basic_ack(delivery_tag=_method.delivery_tag)
        print(" [x] Done")
